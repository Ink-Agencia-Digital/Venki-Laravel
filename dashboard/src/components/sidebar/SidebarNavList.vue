<template>
    ...
    >
        ...
        >
            <span v-if="menu.badge" class="badge pull-right">{{
                menu.badge
            }}</span>
            ...
            <span>
                {{ menu.title }}
                <span v-if="menu.label" class="label label-theme">{{
                    menu.label
                }}</span>
                <i
                    v-if="menu.highlight"
                    class="fa fa-paper-plane text-theme"
                ></i>
            </span>
        </a>
        <ul
            ...
            v-bind:style="{
                marginTop: pageOptions.pageSidebarMinified
                    ? -(scrollTop + 40) + 'px'
                    : '',
            }"
        >
...
export default {
    ...
    methods: {
        ...
        expand: function () {
            if (this.stat == "") {
                this.stat = this.subIsActive(this.menu.path)
                    ? "collapse"
                    : "expand";
            } else {
